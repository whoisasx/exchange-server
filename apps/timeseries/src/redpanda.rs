...
use crate::{settings::TimeseriesSettings, worker::TimeseriesWorker};
...
impl TimeseriesQueue {
    pub async fn new(settings: &TimeseriesSettings) -> Result<Self, QueueError> {
        let brokers = parse_brokers(&settings.redpanda_brokers)?;
        let client = ClientBuilder::new(brokers)
            .client_id("exchange-timeseries")
            .build()
            .await?;
    ...
    pub async fn run(self, worker: TimeseriesWorker) -> Result<(), QueueError> {
...
async fn run_engine_event_partition(
    ...
    println!("timeseries consuming '{topic}' partition {partition} from offset {next_offset}");
...
impl From<crate::worker::TimeseriesError> for QueueError {
    fn from(error: crate::worker::TimeseriesError) -> Self {
...
impl fmt::Display for QueueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Task(error) => write!(f, "timeseries queue task failed: {error}"),
            ...
            Self::Worker(error) => write!(f, "timeseries worker failed: {error}"),
        }
    }
}

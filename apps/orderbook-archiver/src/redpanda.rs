...
use crate::{settings::OrderbookArchiverSettings, worker::OrderbookArchiverWorker};

const FETCH_BYTES: Range<i32> = 1..52_428_800;
const FETCH_MAX_WAIT_MS: i32 = 500;
const IDLE_SLEEP: Duration = Duration::from_millis(100);

pub struct OrderbookArchiverQueue {
    engine_events_topic: String,
    engine_event_partitions: Vec<Arc<PartitionClient>>,
}

impl OrderbookArchiverQueue {
    pub async fn new(settings: &OrderbookArchiverSettings) -> Result<Self, QueueError> {
        let brokers = parse_brokers(&settings.redpanda_brokers)?;
        let client = ClientBuilder::new(brokers)
            .client_id("exchange-orderbook-archiver")
            .build()
            .await?;
    ...
    pub async fn run(self, worker: OrderbookArchiverWorker) -> Result<(), QueueError> {
        ...
        for partition_client in self.engine_event_partitions {
            ...
            tasks.spawn(async move {
                run_engine_event_partition(topic, partition_client, worker).await
            });
        }
...
async fn run_engine_event_partition(
    ...
    println!(
        "orderbook archiver consuming '{topic}' partition {partition} from offset {next_offset}"
    );
    ...
    loop {
        ...
        for record in records {
            ...
            worker
                .process_engine_event(event, &topic, partition, offset, record_next_offset)
                .await?;
            next_offset = record_next_offset;
        }
    }
...
impl From<crate::worker::OrderbookArchiverError> for QueueError {
    fn from(error: crate::worker::OrderbookArchiverError) -> Self {
...
impl fmt::Display for QueueError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::Task(error) => write!(f, "orderbook archiver queue task failed: {error}"),
            ...
            Self::Worker(error) => write!(f, "orderbook archiver worker failed: {error}"),
        }
    }
}

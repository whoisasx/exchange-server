...
The script starts local Postgres and Redpanda with Docker Compose, creates the stream topics, starts `wallet`, `projector`, `ledger`, `fake-engine`, `ws`, and `server`, then drives the REST and websocket flow with `apps/e2e-smoke`.
...
- Publishes `OrderOpened`, `OrderCancelled`, and `TradeExecuted` events to `engine.events` keyed by `market_id`.
...
5. Confirm the server receives engine replies, wallet receives settlement events, projector writes the order/fill read models, ledger writes audit entries, and websocket clients receive account plus market updates.

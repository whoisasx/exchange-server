...
- Publishes `OrderOpened`, `OrderCancelled`, `TradeExecuted`, and `OrderBookDelta` events to `engine.events` keyed by `market_id`, with per-market `engine_sequence` and engine-assigned `engine_timestamp_ms`.
